...
class ActivityRecognition{
  ...
  void startDrivingDetection(){
    ...
    if(active != null ) {
      if(activeSubscription != null && activeSubscription.isPaused){
        ...
        activeSubscription = active.listen((action) {
          ...
          if (action.type == "IN_VEHICLE")
            DrivingScore = 100;
          if (action.type == "ON_BICYCLE")
            DrivingScore = 0;
          if (action.type == "ON_FOOT")
            DrivingScore = 0;
          if (action.type == "RUNNING")
            DrivingScore = 0;
          if (action.type == "WALKING")
            DrivingScore = 0;
        });
      }
    }
    ...
    Timer.periodic(Duration(seconds: 2),(DrivingCheck) {
      if(userActivity.type == "STILL" && DrivingScore > 0)
        DrivingScore -= 1;
      if(userActivity.type == "TILTING" && DrivingScore > 0)
        DrivingScore -= 1;
